...
- `apps/` holds the externally exported applications, assembled from Client / Host mixtures.
    ...
    - `apps/cli` (`@solsticeai/equinox`) dispatches commands: `dsh web` = Host + webserver + the built `dsh-web-frontend` dist; `dsh --profile headless` = [a direct core Agent/Session entry point](2026-08-09-headless-direct-core-entry-point.md), with zero Host, HTTP, or browser layer.
