...
1. `@solsticeai/equinox-web` (`packages/web/web`) owns `ctx.web`, provider registration, provider selection, shared request/result vocabulary, and web-specific errors.
2. Provider packages implement concrete backends and register capabilities with `ctx.web`, for example `@solsticeai/equinox-web-search-exa`, `@solsticeai/equinox-web-search-perplexity`, `@solsticeai/equinox-web-search-deepseek`, and `@solsticeai/equinox-web-fetch-http`.
3. `@solsticeai/equinox-tool-web` (`packages/web/tool-web`) owns the model-facing `web_search` and `web_fetch` tool schemas, prompt sections, argument validation, result formatting, and tool-owned presentation over `ctx.web`.
...
```text
@solsticeai/equinox-tool-web  --depends on-->  @solsticeai/equinox-web  <--depends on--  @solsticeai/equinox-web-search-exa
        consumer                                 interface                       implementation
                                                                 <--depends on--  @solsticeai/equinox-web-search-perplexity
                                                                                  implementation
                                                                 <--depends on--  @solsticeai/equinox-web-search-deepseek
                                                                                  implementation
                                                                 <--depends on--  @solsticeai/equinox-web-fetch-http
                                                                                  implementation
```
...
```mermaid
flowchart LR
  exa["@solsticeai/equinox-web-search-exa"] -->|registerSearchProvider| web["@solsticeai/equinox-web / ctx.web"]
  perplexity["@solsticeai/equinox-web-search-perplexity"] -->|registerSearchProvider| web
  deepseek["@solsticeai/equinox-web-search-deepseek"] -->|registerSearchProvider| web
  fetchLocal["@solsticeai/equinox-web-fetch-http"] -->|registerFetchProvider| web
  toolWeb["@solsticeai/equinox-tool-web"] -->|search/fetch| web
  toolWeb -->|ctx.tools.register| webSearch["tool: web_search"]
  toolWeb -->|ctx.tools.register| webFetch["tool: web_fetch"]
```

`@solsticeai/equinox-web` depends only on Cordis and low-level harness support. It declares `ctx.web`, provider interfaces, request/result types, the provider availability contract, and error codes. It does not import tool, agent, session, LLM, or provider packages.
...
`@solsticeai/equinox-tool-web` depends on `@solsticeai/equinox-web`, `@solsticeai/equinox-tools`, `@solsticeai/equinox-system-prompt`, and Cordis. It never imports concrete provider packages.
...
```ts
import type { WebFetchRequest, WebFetchResult, WebSearchRequest, WebSearchResult } from '@solsticeai/equinox-web'
...
- id: web-search-perplexity
  name: '@solsticeai/equinox-web-search-perplexity'

- id: web-search-deepseek
  name: '@solsticeai/equinox-web-search-deepseek'
