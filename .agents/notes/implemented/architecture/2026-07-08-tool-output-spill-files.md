...
| Package | Role |
|---|---|
| `@solsticeai/equinox-spill` | Interface: `ctx.spillStore`, vocabulary types, no storage implementation. |
| `@solsticeai/equinox-spill-local` | Local backend: private, session-scoped file storage on the host filesystem. |
| `@solsticeai/equinox-spill-policy` | Tool-result policy plugin: wraps final text results after dispatch and replaces oversized results with a retained preview plus a spill locator. |
...
- `@solsticeai/equinox-output-retention` owns preview mechanics (`TextRetainer`, `ItemRetainer`, and omitted metadata).
- `@solsticeai/equinox-spill` owns saving final text and returning a locator plus retrieval hint.
- `@solsticeai/equinox-spill-policy` applies the default final-result policy in the tool pipeline, composing the two.
