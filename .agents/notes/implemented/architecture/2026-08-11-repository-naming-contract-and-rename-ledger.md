...
`SDK` means the JSON-RPC-based client/server protocol used by the supported Python and TypeScript SDKs. The repository keeps `@solsticeai/equinox-sdk-client`, `@solsticeai/equinox-sdk-protocol`, and the wire identity `deepseek-harness-sdk-runtime`; the JSON-RPC server belongs to the same family. DeepSeek Harness itself is not an SDK, and the removed project generator, launcher, helper, and launcher telemetry packages stay absent.
...
|---|---|---|
| `@solsticeai/equinox-jsonrpc` | `@solsticeai/equinox-sdk-jsonrpc-server` | It is the server half of the SDK protocol. `jsonrpc` alone names an encoding; `sdk-jsonrpc-server` gives the family, mechanism, and role. |
| `HarnessSdkServer` | `HarnessSdkJsonRpcServer` | The class is one JSON-RPC server implementation, not every possible SDK server. |

Keep `@solsticeai/equinox-sdk-client`, `@solsticeai/equinox-sdk-protocol`, and `deepseek-harness-sdk-runtime`. Exclude `@solsticeai/create-sdk`, `@solsticeai/equinox-scripts`, `@solsticeai/equinox-helper`, and `@solsticeai/equinox-telemetry`; the separate removal decision deletes them and their support graph.
...
| `@solsticeai/equinox-bash`, `ctx.bash` | `@solsticeai/equinox-shell`, `ctx.shell` | PowerShell already implements this seam. The capability is shell execution, not Bash. |
...
| `@solsticeai/equinox-bash-env`, `ctx.bashEnv`, `BashEnvRegistry` | `@solsticeai/equinox-shell-env`, `ctx.shellEnv`, `ShellEnvRegistry` | The environment registry is shared by Bash and PowerShell tools. |
...
| `@solsticeai/equinox-pty`, `ctx.pty`, `PtyService` | `@solsticeai/equinox-terminal`, `ctx.terminals`, `TerminalSessionService` | Callers manage multiple named terminal sessions. They do not allocate raw PTYs through this service. |
...
| `@solsticeai/equinox-pty-local`, `LocalPtyBackend` | `@solsticeai/equinox-terminal-bash`, `BashTerminalBackend` | The provider depends on Bash prompt and shell behavior. `local` hides the actual dialect. |
| `@solsticeai/equinox-tool-pty` | `@solsticeai/equinox-tool-terminal` | The model-facing tools are already `terminal_*`; the package should use the same product noun. |
...
| Former | Current | Reason |
|---|---|---|
| `@solsticeai/equinox-lsp-local` | `@solsticeai/equinox-lsp-stdio` | The provider speaks LSP over stdio through replaceable filesystem and subprocess services. It is not necessarily local. |
...
| `@solsticeai/equinox-tasks`, `ctx.tasks`, `TaskService` | `@solsticeai/equinox-jobs`, `ctx.jobs`, `JobRegistry` | The service registers, owns, observes, waits for, and cancels multiple background jobs. It is a registry, not a general task service. |
...
| `@solsticeai/equinox-tasks-local`, `LocalTaskService` | `@solsticeai/equinox-jobs-local`, `LocalJobRegistry` | This is the process-local provider of the job registry. Here `local` is meaningful because the jobs and callbacks live in one process. |
| `@solsticeai/equinox-tool-tasks` | `@solsticeai/equinox-tool-jobs` | The consumer controls the job registry and should use the same domain noun. |
...
| `@solsticeai/equinox-client-ui-task`, `client/ui-task/` | `@solsticeai/equinox-client-ui-jobs`, `client/ui-jobs/` | The client package presents the background-job collection. It is not one user task. |
...
| Former | Current | Reason |
|---|---|---|
| `@solsticeai/equinox-client-ui-slash`, `ui-slash/` | `@solsticeai/equinox-client-ui-input-trigger`, `ui-input-trigger/` | The client handles `/`, `@`, keyboard arbitration, candidate menus, and programmatic launch. It is not only slash commands. |
...
| `@solsticeai/equinox-agent-tool-mode`, plugin `tool-mode` | `@solsticeai/equinox-agent-tool-presentation`, plugin `tool-presentation` | The plugin changes how tools are presented to the model. It does not change execution behavior. Keep local `Config.mode` and `ToolPresentationMode`. |
...
| `@solsticeai/equinox-permission`, `ctx.permission`, `PermissionService` | `@solsticeai/equinox-permission-presets`, `ctx.permissionPresets`, `PermissionPresetService` | The service selects and persists presets. Sandbox and approval services enforce the result. |
| `@solsticeai/equinox-client-ui-permission` | `@solsticeai/equinox-client-ui-permission-presets` | The UI edits and selects permission presets. |
...
| `@solsticeai/equinox-user-interaction`, `user-interaction/` | `@solsticeai/equinox-user-questions`, `user-questions/` | The seam supports question batches and answers only. Approval, commands, and directory picking are separate interaction seams. |
| `ctx.userInteraction`, `UserInteractionService`, `UserInteractionProvider`, `UserInteractionError` | `ctx.userQuestions`, `UserQuestionService`, `UserQuestionProvider`, `UserQuestionError` | These names state the one supported interaction form. Keep `AskUserQuestion*`, the `ask_user_question` tool, and `@solsticeai/equinox-tool-ask-user`. |
...
| Former | Current | Reason |
|---|---|---|
| `packages/typert/type-meta/`, `@solsticeai/equinox-type-meta` | `typert/protocol/`, `@solsticeai/equinox-typert-protocol` | The package owns the Typert Remote protocol, decorators, bindings, codecs, lookups, and context contracts. It is not generic type metadata. |
...
Keep `@solsticeai/equinox-tools` and `ctx.tools`. Keep `@solsticeai/equinox-api-gateway`, its `gateway/` folder, `ctx.typertGateway`, and `TypertGatewayService`; that service is a real API gateway. Its internal `TypeRT*` identifiers still follow the `Typert*` spelling rule.

### Workspace instructions, telemetry, identity, and launch environment

| Former | Current | Reason |
|---|---|---|
| Host `ctx.workspace` | Host `ctx.workspaceRegistry` | `WorkspaceRegistry` owns multiple workspaces, but Client `ctx.workspaces` already has an incompatible type. Both declarations merge into the same Cordis `Context` interface at compile time even though their runtime contexts are separate. The role suffix states the host service and avoids that collision. Keep `@solsticeai/equinox-workspace`, `WorkspaceRegistry`, `Workspace`, and `workspace.*` wire names. |
| `@solsticeai/equinox-workspace-context`, `context/workspace-context/` | `@solsticeai/equinox-agent-instructions`, `context/agent-instructions/` | The package loads hierarchical `AGENTS.md` and `CLAUDE.md` files for the agent. It is not general workspace context. |
...
| `session/user-id/`, `@solsticeai/equinox-user-id` | `identity/anonymous-user-id/`, `@solsticeai/equinox-anonymous-user-id` | The value is a random correlation id shared by telemetry, feedback, and DeepSeek requests. It is neither a Session concern nor an authenticated user identity. |
...
| `util/environment/`, `@solsticeai/equinox-environment` | `util/launch-environment/`, `@solsticeai/equinox-launch-environment` | The package captures one immutable layered snapshot at launch. It is not a general environment API. |
...
| Former | Current | Reason |
|---|---|---|
| `@solsticeai/equinox-tool-schedule`, `schedule/tool-schedule/`, plugin `tool-schedule` | `@solsticeai/equinox-schedule`, `schedule/schedule/`, plugin `schedule` | The package owns the durable Schedule domain, persistence barriers, management tools, timers, follow-ups, and runtime lifecycle. `tool-` describes only one part. |
...
| `WorkflowService`, `ctx.workflows` | `WorkflowEngine`, `ctx.workflowEngine` | One engine parses and executes workflow programs. The plural key wrongly suggests a registry. Keep `@solsticeai/equinox-workflow` and workflow events and tools. |
| `@solsticeai/equinox-workflow-workerthread`, `WorkerWorkflowEngine` | `@solsticeai/equinox-workflow-worker-thread`, `WorkerThreadWorkflowEngine` | `worker thread` is the precise Node mechanism and the repository spelling uses the full words. |
| `@solsticeai/equinox-goal-session`, `goal/goal-session/` | `@solsticeai/equinox-goal-round-driver`, `goal/goal-round-driver/` | The plugin drives same-session Goal Rounds. It neither stores goals nor defines sessions. Keep `GoalService`, goal source, events, and contracts. |
| `packages/compact/` | `packages/compaction/` | The group is a noun-domain family. `compact` remains the user command verb. |
| `@solsticeai/equinox-compact`, `ctx.compact`, `CompactService` | `@solsticeai/equinox-compaction`, `ctx.compaction`, `CompactionEngine` | The object runs the compaction algorithm and lifecycle. It is an engine, not a generic service. |
...
| `@solsticeai/equinox-compact-basic`, `BasicCompactService`, public `BasicCompact*` | `@solsticeai/equinox-compaction-basic`, `BasicCompactionEngine`, corresponding `BasicCompaction*` | `basic` is plain but honest. `compaction-llm` adds no information because LLM use is already part of the current implementation family. |
| `@solsticeai/equinox-compact-tool-result-prune`, `ToolResultPruneService`, `ctx.toolResultPrune` | `@solsticeai/equinox-compaction-tool-result-pruner`, `ToolResultPruner`, `ctx.toolResultPruner` | The plugin is an actor that prunes tool results. The noun `pruner` names that role. |
...
| `@solsticeai/equinox-settings-local`, `SettingsLocal` | `@solsticeai/equinox-settings-file`, `FileSettingsProvider` | The implementation is file-backed through the filesystem seam. `file` states the mechanism; `local` does not. |
...
| Former | Current | Reason |
|---|---|---|
| `HttpServerService`, `ctx.httpServer` | `WebServer`, `ctx.webServer` | The server owns HTTP routes and WebSocket upgrade routes. `Web` leaves room for both; `Http` is too narrow here. Keep `packages/host/webserver`, `@solsticeai/equinox-host-webserver`, `WebRoute`, and `WebUpgradeRoute`. |
...
| `@solsticeai/equinox-session-title-first-message-llm`, cadence `first-message` | `@solsticeai/equinox-session-title-first-prompt-llm`, cadence `first-prompt` | The trigger is the first user prompt, not any message in the session log. |
| `@solsticeai/equinox-session-title-all-messages-llm`, cadence `all-user-messages` | `@solsticeai/equinox-session-title-all-prompts-llm`, cadence `all-prompts` | The backend refreshes from user prompts. `all messages` wrongly includes assistant and tool events. |
| `@solsticeai/equinox-code-runtime-worker`, `WorkerCodeRuntime` | `@solsticeai/equinox-code-runtime-worker-thread`, `WorkerThreadCodeRuntime` | The implementation uses a Node worker thread. `worker` alone is too broad. |
...
| Former | Current | Reason |
|---|---|---|
| `@solsticeai/equinox-fs-policy` | `@solsticeai/equinox-fs-observation-policy` | The package defines which filesystem observations authorize later effects. It is not the complete filesystem or sandbox policy. |
...
| `@solsticeai/equinox-skill-local`, `LocalSkillProvider`, provider id `local` | `@solsticeai/equinox-skill-filesystem`, `FileSystemSkillProvider`, provider id `filesystem` | The provider discovers skill files through `ctx.fs`, which can be local or remote. The mechanism is filesystem access, not locality. |
...
| `@solsticeai/equinox-subagent-spawn`, `SpawnProvider` | `@solsticeai/equinox-subagent-spawn-in-process`, `SpawnInProcessProvider` | This provider starts a child agent in the current process. The configured provider id remains `spawn`. |
| `@solsticeai/equinox-subagent-fork`, `ForkProvider` | `@solsticeai/equinox-subagent-fork-in-process`, `ForkInProcessProvider` | This provider forks an agent in the current process. The configured provider id remains `fork`. |
| `@solsticeai/equinox-subagent-inprocess`, `subagent-inprocess/` | `@solsticeai/equinox-subagent-in-process-driver`, `subagent-in-process-driver/` | The package contains common in-process driving logic, not a third provider. |
...
| `@solsticeai/equinox-web-fetch-local`, `LocalFetchProvider`, `LocalFetchLimits`, provider id `local-http` | `@solsticeai/equinox-web-fetch-http`, `HttpFetchProvider`, `HttpFetchLimits`, provider id `http` | This provider performs direct HTTP fetches. `local` says where code happens to run, not which mechanism it provides. |

Keep `@solsticeai/equinox-subagent-dsh-sdk`, its provider id `dsh-sdk`, external ACP, Codex, and Claude Code provider families, the subagent tool package names, the main filesystem package and backends, filesystem tools and events, and the skill badge and tool packages.

### Hooks, guards, plan mode, extensions, and diagnostics

| Former | Current | Reason |
|---|---|---|
| `@solsticeai/equinox-hooks-claude`, `ClaudeHookConfig`, `parseClaudeConfig`, dialect `claude` | `@solsticeai/equinox-hooks-claude-code`, `ClaudeCodeHookConfig`, `parseClaudeCodeConfig`, dialect `claude-code` | The hook bridge targets Claude Code, not every Anthropic or Claude product. |
| `@solsticeai/equinox-repeat-tool-guard`, plugin/source `repeat-tool-guard` | `@solsticeai/equinox-repeat-tool-reminder`, plugin/source `repeat-tool-reminder` | The plugin adds a model reminder. It does not block or enforce a guard decision. |
| `@solsticeai/equinox-timeout-policy` | `@solsticeai/equinox-tool-call-timeout-policy` | The full `tool-call` qualifier names what the policy limits without calling the plugin a model-facing tool. Keep its `guard/timeout-policy/` directory and plugin id `timeout-policy`; the `packages/*/tool-*` catalog convention still applies only to packages that register tools. |
...
| `InvariantService` | `InvariantRegistry` | The object owns registered invariant checks. Keep `@solsticeai/equinox-invariants` and `ctx.invariants`. |
...
| Former | Current | Reason |
|---|---|---|
| `util/paths/`, `@solsticeai/equinox-paths` | `util/home-paths/`, `@solsticeai/equinox-home-paths` | The helpers resolve paths under the Harness home. They are not a general path library. Keep the individual function names when they already state the returned path. |
| `util/retention/`, `@solsticeai/equinox-retention` | `util/output-retention/`, `@solsticeai/equinox-output-retention` | The policy retains command and tool output. It is not a general data-retention framework. |
| `E2BSandboxService` | `E2BRuntime` | The class creates, reuses, and disposes the E2B execution environment used by filesystem and subprocess adapters. It is broader than one sandbox handle and narrower than a generic owner. Keep `@solsticeai/equinox-e2b`, `ctx.e2b`, and the `e2b/` group. |
| `@solsticeai/equinox-frontend-static` | `@solsticeai/equinox-host-frontend-static` | The package is the Host plugin that serves the frontend assets. The prefix distinguishes it from frontend application code. |
...
| `@solsticeai/equinox-jsonrpc-demo`, `@solsticeai/equinox-sdk-jsonrpc-demo`, `@solsticeai/equinox-sdk-python-runtime` | removed | The Python runtime packages the existing `@solsticeai/equinox` CLI and its `sdk` profile; a private application package would recreate a second launcher. |
...
| `@solsticeai/equinox-acp-demo` | `@solsticeai/equinox-acp-app` | The package is the ACP profile's application bundle, not a standalone demo bin. |
...
| `@solsticeai/equinox-frontend` | `@solsticeai/equinox-web-frontend` | The application is the web frontend. Keep its physical `apps/web/` folder. |
...
| `@solsticeai/equinox-client-ui-model` | `@solsticeai/equinox-client-ui-model-selection` | The package controls the model selection for a session. The singular `model` name is too broad. |
...
| `@solsticeai/equinox-client-ui-models` | `@solsticeai/equinox-client-ui-settings-models` | This package owns the Models settings panel. Keep `ModelsSettingsStore`; it holds one settings view model with data operations and subscriptions and is a real store. |
| `@solsticeai/equinox-client-ui-plugin-config`, `client/ui-plugin-config/` | `@solsticeai/equinox-client-ui-settings-plugins`, `client/ui-settings-plugins/` | This package owns the Plugins settings section, not a general plugin-configuration system. The target joins the `ui-settings-*` family and uses the section's plural product name. |
...
| `@solsticeai/equinox-client-ui-plugins`, `client/ui-plugins/`, Loader id `ui-plugins`, `client-ui-plugins-invariant` | `@solsticeai/equinox-client-ui-settings-plugin-inventory`, `client/ui-settings-plugin-inventory/`, Loader id `ui-settings-plugin-inventory`, `client-ui-settings-plugin-inventory-invariant` | This later package owns the read-only Plugin Inventory tab in the Plugins settings section. `ui-plugins` is too broad and does not distinguish the inventory from editable plugin settings. |
...
| `@solsticeai/equinox-client-ui-feedback`, `client/ui-feedback/`, Loader id `ui-feedback`, `client-ui-feedback-invariant` | `@solsticeai/equinox-client-ui-message-feedback`, `client/ui-message-feedback/`, Loader id `ui-message-feedback`, `client-ui-message-feedback-invariant` | This package presents ratings and notes for assistant messages through the `messageFeedback` Remote. The old name also appears to cover command feedback and any later feedback UI. It does not. |
...
| `@solsticeai/equinox-client-ui-question` | `@solsticeai/equinox-client-ui-user-questions` | The UI presents the user-question seam, not an arbitrary question domain. |
| `@solsticeai/equinox-client-ui-command`, `ui-command/` | `@solsticeai/equinox-client-ui-commands`, `ui-commands/` | The package presents and runs a collection of commands. |
| `@solsticeai/equinox-client-ui-directory-picker`, `client/ui-directory-picker/`, Loader id `ui-directory-picker`, `client-ui-directory-picker-invariant` | `@solsticeai/equinox-client-ui-directory-picker-browse`, `client/ui-directory-picker-browse/`, Loader id `ui-directory-picker-browse`, `client-ui-directory-picker-browse-invariant` | The Client packages now contain separate `browse` and `native` directory-picker presentations. The unqualified package is the browse implementation, not their shared definition. The target matches the Host backend family and changes no boundary. |
...
- Keep `@solsticeai/equinox-api-gateway`, `ctx.typertGateway`, and `TypertGatewayService`.
- Keep session projection names. A projection is not only a reducer function.
- Keep `@solsticeai/equinox-session-stats`, `sessionStats`, and `SessionStatsProjection`. They accurately name whole-session statistics and the maintained read model that carries them.
...
- Keep `@solsticeai/equinox-client-ui-directory-picker-native`; its suffix names the native-chooser presentation beside the renamed `-browse` variant. Keep `SURFACE_PACKAGES`; within the directory-picker auto selector it is the package map for the Client presentation half, contrasted with `BACKEND_PACKAGES`.
- Keep `@solsticeai/equinox-host-plugin-inventory`, `ctx.pluginInventory`, the `pluginInventory/list` Remote, and the `PluginInventory*` payload types. They accurately name the Host-owned read-only inventory; only the adapter class and the overly broad Client presentation names change.
