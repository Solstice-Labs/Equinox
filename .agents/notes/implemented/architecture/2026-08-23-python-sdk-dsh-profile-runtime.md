...
The runtime executable packages `@solsticeai/equinox` and runs its ordinary command grammar. The Python client selects `--profile sdk` by default, forwards ordered absolute `--patch` paths, and may select another `dsh` executable or profile. The runnable minimal example selects the shipped `sdk-minimal` profile. The private `@solsticeai/equinox-sdk-python-runtime` application package and checked-in runtime `cordis.yml` do not exist. JSON-RPC serving remains the `@solsticeai/equinox-sdk-app` bundle and `@solsticeai/equinox-sdk-jsonrpc-server` plugin, not a Python-owned boot path.
