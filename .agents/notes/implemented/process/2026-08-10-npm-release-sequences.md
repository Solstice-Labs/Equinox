...
|---|---|---|
| `@solsticeai/cordis` | 4.0.0-rc.7 | 4.0.1 |
| `@solsticeai/cordis-plugin-loader` | 1.0.0-rc.5 | 1.0.1 |
| `@solsticeai/cosmokit` | 1.8.1 | 1.8.2 |
| `@solsticeai/schemastery` | 3.18.0 | 3.18.1 |
| `@solsticeai/cordis-plugin-hmr` | 1.0.15 | 1.0.16 |
| `@solsticeai/cordis-plugin-include` | 1.0.4 | 1.0.5 |
| `@solsticeai/cordis-plugin-timer` | 1.1.2 | 1.1.3 |
| `@solsticeai/cordis-plugin-group` | 1.0.0 | 1.0.1 |
| `@solsticeai/cordis-plugin-logger-console` | 1.0.0 | 1.0.1 |
...
`scripts/check-workspace-constraints.ts` requires the protocol, so a new package cannot reintroduce a hand-written range; the invariant-companion rule requires `workspace:^` for `@solsticeai/equinox-invariants` for the same reason.
...
**Selecting a subset by entry closure.** Crawling `dependencies` from `@solsticeai/equinox` and `@solsticeai/equinox-web-frontend` yields 156 packages, 61 fewer than the whole set. But this repository's plugins are mounted by name from `cordis.yml` rather than imported: `vendor/cordis-plugin-group` and `vendor/cordis-plugin-logger-console` fall outside the dependency closure while being required at runtime. Selecting by code dependency fails as "the consumer installs it and it will not start", and it would need a standing proof that no mounted package was missed. Under a private scope the extra packages are invisible outside the organization. `python/`, `docs/`, and `website/` are not release-family members.
