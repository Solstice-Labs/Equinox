...
- `@solsticeai/equinox-tui` owns terminal-interactive execution. It rejects non-TTY streams before Loader boot; `apps/cli/config/base.cordis.yml` plus the `tui.cordis.yml` overlay own the complete coding composition, with PTY plus terminal-snapshot coverage in `apps/cli/tests/`.
...
- [`dsh --profile acp`](../../../../apps/cli/README.md) and `@solsticeai/equinox-sdk-jsonrpc-server` own their framed protocol integrations.
