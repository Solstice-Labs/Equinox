...
The repository carried an unreleased developer-project product with no consumers. `@solsticeai/create-sdk` generated an editable Cordis project, `@solsticeai/equinox-scripts` supplied its `dsh-sdk` development, build, start, configuration, and plugin-install commands, `@solsticeai/equinox-helper` coordinated feature definitions and multi-file project edits, and `@solsticeai/equinox-telemetry` reported launcher activity. The design aimed to keep generated projects editable while giving creation and later configuration one definition of dependencies, Cordis entries, environment placeholders, and owned files.
