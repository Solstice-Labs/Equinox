...
Compaction is split between `@solsticeai/equinox-compaction`, which owns an abstract two-method service and shared types, and `@solsticeai/equinox-compaction-basic`, which owns the only complete provider. Shipped configurations load only the basic package, and no production package independently consumes the Service Definition package except that provider.
...
Move the basic implementation into `@solsticeai/equinox-compaction` and remove `@solsticeai/equinox-compaction-basic`. Keep `ctx.compaction`, `CompactionResult`, the shared transcript and tool-pairing helpers, the existing configuration, and the concrete compaction algorithm in one package.
...
- `@solsticeai/equinox-compaction-basic` and its workspace/package metadata are removed.
- `@solsticeai/equinox-compaction` owns the current configuration, plugin class, algorithm, types, events, and shared helpers.
