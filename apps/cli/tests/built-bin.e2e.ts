...
import { startMockLlmServer } from '@solsticeai/equinox-llm-mock-server'
...
function createEnvironmentProbeProfile(home: string, project: string): void {
  ...
  writeFileSync(join(profileDir, 'package.json'), JSON.stringify({
    ...
    private: true,
    dependencies: {},
    dsh: { profile: { bundles: ['@solsticeai/equinox-base'] } },
  }, undefined, 2))
  writeFileSync(join(profileDir, 'cordis.patch.yml'), [
    '- insert:',
 ...
 * `@solsticeai/equinox-cmdline` and `commander` through the profile module
...
function createStartupFixture(): StartupFixture {
  ...
  writeFileSync(join(bundleDir, 'startup.mjs'), [
    ...
    "import { parseCmdline } from '@solsticeai/equinox-cmdline'",
...
describe.skipIf(!existsSync(dshBin))('dsh BUILT bin (node lib/bin.js, no tsx)', () => {
  ...
  it('reports SDK startup failure when stdin reaches EOF first', async () => {
    ...
    writeFileSync(patch, [
      ...
      '      name: "@solsticeai/equinox-missing-sdk-startup-plugin"',
      '',
    ].join('\n'))
    try {
      ...
      expect(result.stderr).toContain('@solsticeai/equinox-missing-sdk-startup-plugin')
    } finally {
      rmSync(home, { recursive: true, force: true })
    }
  ...
  it('activates a dependency that gained dsh.bundle in a later update', async () => {
    ...
    try {
      ...
      expect(manifest.dsh.profile.bundles).toEqual(['@solsticeai/equinox-base', 'late-bundle'])
    } finally {
      rmSync(home, { recursive: true, force: true })
    }
  }, SPAWN_TIMEOUT_MS * 2 + 30_000)
  ...
  describe('config dump', () => {
    ...
    it('prints the web profile bundle layers without a user layer', async () => {
      ...
      expect(code).toBe(0)
      expect(stderr).toBe('')
      expect(stdout).toContain("name: '@solsticeai/equinox-agent-loop'")
      expect(stdout).toContain('agents: []')
      expect(stdout).toContain('# == @solsticeai/equinox-base')
      expect(stdout).toContain("name: '@solsticeai/equinox-host-webserver'")
    ...
    it('prints the exact standalone sdk-minimal tree without dsh-base', async () => {
      ...
      expect(stdout).toContain('# == @solsticeai/equinox-sdk-minimal')
      expect(stdout).not.toContain('@solsticeai/equinox-base')
      expect(stdout).not.toContain('@solsticeai/equinox-web-app')
    }, SPAWN_TIMEOUT_MS * 2 + 30_000)
