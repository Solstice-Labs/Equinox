 ...
 *
 * @module @solsticeai/equinox-api-settings-controller
 */
...
import {
  ...
  openNativePath,
  openNativeTextFile,
} from '@solsticeai/equinox-native-command'
import type { SettingsDescriptor, SettingsPathOp, SettingsProvider } from '@solsticeai/equinox-settings'
import type {
...
export class SettingsController extends TypertRemoteService {
  ...
  private provider(): SettingsProvider {
    ...
    if (settings === undefined) {
      throw new RemoteError(
        'gateway/internal',
        'settings service is absent: this deployment does not mount a settings provider (e.g. @solsticeai/equinox-settings-file) in its composition',
        {},
      )
    }
    return settings
