 ...
 * Everything else under @solsticeai/* is either a module-table entry
 * (external) or a leak the purity gate rejects.
 */
export const INLINE_SAFE = /^(?:@solsticeai\/equinox-(?:file-reference|session|llm|tools|brand|deque|typert-protocol|util-crypto|util-values|util-workspace-path)(?:\/|$)|@solsticeai\/equinox-token-meter\/client$|@solsticeai\/equinox-agent-presets\/display$)/

/**
 * Vendored framework libraries: rescoped into @solsticeai, so the gate below
 ...
 */
const VENDORED_LIBRARY = /^@solsticeai\/(cosmokit|schemastery)(\/|$)/

/** Generated descriptor/codec contribution with no shared runtime identity. */
const GENERATED_REMOTE = /^@solsticeai\/equinox-[a-z0-9]+(?:-[a-z0-9]+)*\/remote$/
...
function clientConfig(id: string, entry: string): UserConfig {
  ...
  return {
    ...
    plugins: [{
      ...
      // inline, and every other @solsticeai value import is a build error — a
      ...
      resolveId(source: string) {
        if (!source.startsWith('@solsticeai/')) return null
