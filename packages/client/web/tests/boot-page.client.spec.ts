...
describe('BootPage', () => {
  ...
  it('lists failed entries', () => {
    const { el, page } = mount()
    page.setState('@solsticeai/equinox-client-ui-layout', 'failed')
    page.setState('ok', 'active')
    page.setState('@solsticeai/equinox-client-ui-tool', 'failed')
    expect(el.textContent).toContain('@solsticeai/equinox-client-ui-layout')
    expect(el.textContent).toContain('@solsticeai/equinox-client-ui-tool')
