...
import * as workspaceContext from '@solsticeai/equinox-agent-instructions'
import LlmRuntime, { createUserMessage, ToolCallId, type Message, type StreamChunk } from '@solsticeai/equinox-llm'
import SessionStore, { Session, SessionId, SessionSeq, SESSION_FORMAT_VERSION, type SessionEvent, type UserMessage } from '@solsticeai/equinox-session'
import SessionProjectionRegistry from '@solsticeai/equinox-session-projection'
import AgentRegistry, { agentEvents, Inbox, type Agent } from '@solsticeai/equinox-agent'
import AgentLoop, { turnBoundaryProjectionDefinition } from '@solsticeai/equinox-agent-loop'
import { FileSystem, FsTargetKey, FsVersion } from '@solsticeai/equinox-fs'
import type {
  FsDirEntry,
  FsEditOutcome,
  FsEditRequest,
...
} from '@solsticeai/equinox-agent-instructions'
import {
...
describe('workspace context instruction discovery', () => {
  ...
  it('labels the default DSH home as ~/.dsh when HOME points at the configured default', async () => {
    ...
    try {
      ...
      const isolated = await import('@solsticeai/equinox-agent-instructions')
