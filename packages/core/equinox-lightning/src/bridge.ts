...
import type { EquinoxConfig, QuantPlan, QuantRule } from '@solsticeai/core'
...
function requireMemBytes(): number {
  try {
    const { totalmem } = require('node:os') as typeof import('node:os')
    return totalmem()
  } catch {
...
export class EquinoxBridge {
  ...
  async dispatchRemote(opts: DispatchOptions, attempt = 0): Promise<ExecResult> {
    ...
    const jobArgs = buildRemoteJobArgs({
      ...
      machine: opts.machine ?? this.config.machine,
      teamspace: this.config.lightningTeamspace,
    })
