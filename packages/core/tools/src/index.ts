 ...
 * execution pipeline.
 * @module @solsticeai/equinox-tools
 */

import { Context, Service } from '@solsticeai/cordis'
import z from '@solsticeai/schemastery'
import { AnonymousEntries, NamedEntries, ScopedLayers, scopeOf, scopeTarget } from '@solsticeai/equinox-scope'
import type { ScopeKey, ScopeLayer, Scoped } from '@solsticeai/equinox-scope'
import type { ToolCallId, ContentBlock, ToolSchema } from '@solsticeai/equinox-llm'
...
import { assertNever, deepFreeze, snapshotJsonValue, type JsonValue } from '@solsticeai/equinox-util-values'
import type { ToolProviderResult } from '@solsticeai/equinox-system-prompt'
import type { CodeRuntime } from '@solsticeai/equinox-code-runtime'
...
// lives in its own UI-facing module; re-export it so `@solsticeai/equinox-tools`
...
declare module '@solsticeai/cordis' {
  ...
  interface Events {
     ...
     * Scope-filtered dispatch (`@solsticeai/equinox-scope`): agent-scoped listeners receive only that agent's calls.
     ...
     * Scope-filtered dispatch (`@solsticeai/equinox-scope`): agent-scoped listeners receive only that agent's dispatches.
     ...
     * Scope-filtered dispatch (`@solsticeai/equinox-scope`): keyed by `exec.agent`.
...
export interface ToolDefinition extends ToolSchema {
   ...
   * Enforced by `@solsticeai/equinox-tool-call-timeout-policy` (a `tools/execute` wrapper); it
 ...
 * @internal
 */
export const TOOL_RUNTIME_SCHEDULER: unique symbol = Symbol('@solsticeai/equinox-tools.scheduler')
...
export class ToolRuntime extends Service {
  ...
  private requireCodeRuntime(mode: ToolPresentationMode): CodeRuntime {
    ...
    if (!runtime) {
      throw new Error(`dsh-tools: mode "${mode}" requires a code runtime — load a ctx.codeRuntime implementation (e.g. @solsticeai/equinox-code-runtime-worker-thread) or set tools mode to "native"`)
    }
