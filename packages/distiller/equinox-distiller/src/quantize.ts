...
import type { EquinoxConfig, ModelProfile, QuantPlan } from '@solsticeai/core'
...
export async function runRequant(options: RequantOptions): Promise<RequantManifest> {
  ...
  const manifest: RequantManifest = {
    ...
    imatrixCmd,
    executed: res.ok,
    note: res.ok ? undefined : res.stderr.slice(0, 300),
  }
