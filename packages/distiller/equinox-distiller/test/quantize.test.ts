...
import { findQuantizeBin, quantTierSummary, runRequant } from '@solsticeai/distiller'
import { loadConfig, type ModelProfile, type QuantPlan } from '@solsticeai/core'
import { EquinoxBridge, type ExecResult } from '@solsticeai/lightning'
...
describe('runRequant', () => {
  ...
  it('runs quantize locally through the executor when an imatrix is provided', async () => {
    ...
    const manifestFile = JSON.parse(readFileSync(`${modelOut}.manifest.json`, 'utf8'))
