 ...
 * @module @solsticeai/equinox-experimental-webworker-packer/src/pack
 */
...
} from '@solsticeai/equinox-experimental-webworker-runtime'
import picomatch from 'picomatch'
import yaml from 'js-yaml'
import { entryListSchema } from '@solsticeai/cordis-plugin-include'
import { REPLACED_EXTERNAL_PACKAGES } from '@solsticeai/equinox-experimental-webworker-runtime/src/node/external_packages/replaced-externals.ts'
import { MODULE_PROXIES, MODULE_PROXY_PREFIXES } from '@solsticeai/equinox-experimental-webworker-runtime/src/module-proxies.ts'
...
export { DEFAULT_ROOT } from '@solsticeai/equinox-experimental-webworker-runtime'
...
function materialize(
  ...
  for (let entry = queue.shift(); entry !== undefined; entry = queue.shift()) {
    ...
    for (const field of ['dependencies', 'peerDependencies'] as const) {
      ...
      // (@solsticeai/cordis is a peerDependency of every harness package),
