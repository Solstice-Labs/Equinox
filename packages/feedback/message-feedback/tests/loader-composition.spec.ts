...
import * as StorageDomain from '@solsticeai/equinox-storage-domain'
import * as StorageJson from '@solsticeai/equinox-storage-json'
import { remoteMethods } from '@solsticeai/equinox-typert-protocol'
...
async function loadComposition(configPath: string): Promise<Context> {
  ...
  const modules = new Map<string, unknown>([
    ...
    ['@solsticeai/equinox-session-persistence-jsonl', JsonlSessionPersistence],
    ['@solsticeai/equinox-storage', Storage],
    ['@solsticeai/equinox-storage-json', StorageJson],
    ['@solsticeai/equinox-storage-domain', StorageDomain],
    ['@solsticeai/equinox-message-feedback', MessageFeedbackService],
  ])
  ctx.loader.internal = {
    version: 'v2',
...
describe('message feedback through a real Loader composition', () => {
  it('persists a checkpointed target and its sidecar across a cold restart', async () => {
    ...
    await writeFile(configPath, [
      ...
      '    compression: none',
      "- name: '@solsticeai/equinox-storage'",
      "- name: '@solsticeai/equinox-storage-json'",
      '  config:',
      `    root: ${JSON.stringify(join(root, 'storage'))}`,
      "- name: '@solsticeai/equinox-storage-domain'",
      '  config:',
      '    backend: json',
