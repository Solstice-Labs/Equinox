...
`src/index.ts` 中的 FIXME 要求确定 `@solsticeai/equinox-timeout-guard` 改名；[改名台账](../../../.agents/notes/implemented/architecture/2026-08-11-repository-naming-contract-and-rename-ledger.zh.md) 已把 `@solsticeai/equinox-tool-call-timeout-policy` 记录为既定名称，因此该 FIXME 已陈旧，待代码清理。
