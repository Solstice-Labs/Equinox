...
import * as CommandInvariant from '@solsticeai/equinox-commands/invariant'
import InvariantRegistry, { InvariantError } from '@solsticeai/equinox-invariants'
import SessionStore, { SessionId, SessionSeq, type Session } from '@solsticeai/equinox-session'
import { CommandId } from '@solsticeai/equinox-commands'
...
describe('command lifecycle invariants', () => {
  ...
  it.each([-1, 1.5, 1])('rejects invalid or non-prior sourceEventSeq %s', async (sourceEventSeq) => {
    ...
    expect(() => {
      ...
      code: 'INVARIANT',
      packageName: '@solsticeai/equinox-commands',
    }))
  })
