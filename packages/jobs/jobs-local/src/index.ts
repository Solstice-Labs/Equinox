 ...
 * @module @solsticeai/equinox-jobs-local
 */

import { Context } from '@solsticeai/cordis'
import z from '@solsticeai/schemastery'
import type { Agent } from '@solsticeai/equinox-agent'
import { AnonymousEntries, ScopedLayers, scopeOf } from '@solsticeai/equinox-scope'
import type { ScopeLayer } from '@solsticeai/equinox-scope'
import { deadline, timeoutOf } from '@solsticeai/equinox-timeout'
import { JobRegistry, JobId } from '@solsticeai/equinox-jobs'
import type {
  JobDoneListener, JobKind, JobOutcome, JobRead, JobSnapshot, JobStart, JobStatus,
  JobsChangedListener,
} from '@solsticeai/equinox-jobs'
 ...
 * `@solsticeai/equinox-jobs` for the ownership, isolation, and lifecycle
...
export class LocalJobRegistry extends JobRegistry {
  ...
  start(spec: JobStart): JobId {
    if (!this.servesOwner(spec.owner)) {
      throw new Error('background jobs unavailable: no job controller serves this agent (load @solsticeai/equinox-tool-jobs in its composition)')
    }
  ...
  private ensureOwnerCleanup(owner: Agent): void {
    ...
    if (agents === undefined) {
      throw new Error('background job ownership requires the agent registry (load @solsticeai/equinox-agent)')
    }
