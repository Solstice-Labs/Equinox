...
import { bindScopeParent, createScope, scopeOf } from '@solsticeai/equinox-scope'
import type { ScopeKey } from '@solsticeai/equinox-scope'
import { JobId } from '@solsticeai/equinox-jobs'
import type { JobHooks, JobKind, JobOutcome, JobSnapshot, JobStart } from '@solsticeai/equinox-jobs'
import LocalJobRegistry, { type Config as JobsConfig } from '@solsticeai/equinox-jobs-local'
...
describe('LocalJobRegistry.start', () => {
  ...
  it('refuses to register while no job controller serves the owner', async () => {
    ...
    expect(() => ctx.jobs.start(producer().spec))
      .toThrow('background jobs unavailable: no job controller serves this agent (load @solsticeai/equinox-tool-jobs in its composition)')
  })
