 ...
 * process-local registry lives in `@solsticeai/equinox-jobs-local`.
 * @module @solsticeai/equinox-jobs
 */
...
export abstract class JobRegistry extends Service {
  constructor(ctx: Context) {
    ...
    if (new.target === JobRegistry) {
      throw new Error('@solsticeai/equinox-jobs is the abstract job registry seam; load an implementation such as @solsticeai/equinox-jobs-local instead')
    }
    super(ctx, 'jobs')
  }
