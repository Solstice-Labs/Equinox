...
import LlmRuntime, { createMessage, createUserMessage, userAgent } from '@solsticeai/equinox-llm'
...
import * as LlmPiAi from '@solsticeai/equinox-llm-pi-ai'
...
async function loadComposition(): Promise<{ ctx: Context; settingsPath: string }> {
  ...
  await writeFile(configPath, [
    ...
    '    debounceMs: 10',
    '- id: llm-pi-ai',
    "  name: '@solsticeai/equinox-llm-pi-ai'",
    '',
  ].join('\n'))
  ...
  const modules = new Map<string, unknown>([
    ...
    ['@solsticeai/equinox-llm-pi-ai', LlmPiAi],
  ])
  ctx.loader.internal = {
    version: 'v2',
