...
import LlmRuntime, { createUserMessage, LlmAdapter, LlmError, resolveRetryPolicy  } from '@solsticeai/equinox-llm'
import type { GenerateOptions, ResolvedRetryPolicy, StreamChunk } from '@solsticeai/equinox-llm'
...
async function loadYaml(lines: readonly string[]): Promise<Context> {
  ...
  const modules = new Map<string, unknown>([
    ...
    ['@solsticeai/equinox-llm-retry', retry],
    ['@solsticeai/equinox-agent-loop', AgentLoop],
  ])
...
describe('real Loader composition', () => {
  ...
  it('loads provider-supplied policy and records recovery through the shipping loop', { timeout: 60_000 }, async () => {
    const loaded = await loadYaml([
      ...
      "- name: '@solsticeai/equinox-llm-retry'",
      "- name: '@solsticeai/equinox-agent-loop'",
    ])
