 ...
 * @module @solsticeai/equinox-plugin-package-inventory-deepseek
 */
...
import type { Entry, EntryTree } from '@solsticeai/cordis-plugin-loader'
...
async function collectActivePluginPackages(
  ...
  if (sessionId !== undefined && ctx.get('agentPresets') !== undefined) {
    ...
    if (agent !== undefined) {
      ...
      const { standingMountFor } = await import('@solsticeai/equinox-agent-presets')
