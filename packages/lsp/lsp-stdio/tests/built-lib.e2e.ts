...
/**
 * Keyless built-artifact smoke: plain Node imports `@solsticeai/equinox-lsp` and
 * `@solsticeai/equinox-lsp-stdio` by name through their exports maps, spawns the fixture server, runs
...
describe.skipIf(!built)('built lib real load path (plain node)', () => {
  it('runs a query through lib/index.js and disposes cleanly, framing over the base protocol', async () => {
    ...
    const script = `
      const { Context } = await import('@solsticeai/cordis')
      const { default: Lsp } = await import('@solsticeai/equinox-lsp')
      const LspLocal = await import('@solsticeai/equinox-lsp-stdio')
      const { default: LocalFileSystem } = await import('@solsticeai/equinox-fs-local')
      const { default: LocalSubprocessRuntime } = await import('@solsticeai/equinox-subprocess-local')
