...
import type { CaptureBackend, LayerMoments, ModelProfile, QuantPlan, QuantRule } from '@solsticeai/core'
import { clamp, drift as driftFn, normalizeScores, quantTierFor, sha256Hex } from '@solsticeai/core'
...
export function buildFingerprint(input: FingerprintInput, options: BuildOptions = {}): ModelProfile {
  const opts = { ...DEFAULT_BUILD, ...options }
  ...
  let quantPlan: QuantPlan = {
    ...
    tokenEmbeddingType: opts.tokenEmbeddingType!,
    outputTensorType: opts.outputTensorType!,
    rules: [],
  }
  let drift = 0
  ...
  if (hasCapture) {
    ...
    quantPlan = quantPlanFromTiers(tiers, {
      baseType: opts.baseType!,
      tokenEmbeddingType: opts.tokenEmbeddingType!,
      outputTensorType: opts.outputTensorType!,
    })
