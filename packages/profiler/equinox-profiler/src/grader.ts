...
function checkNode(value: unknown, schema: Record<string, unknown>, path: string, errors: string[]): void {
  ...
  if (expected === 'object') {
    ...
    const props = isPlainObject(schema.properties) ? (schema.properties as Record<string, Record<string, unknown>>) : {}
    for (const [key, sub] of Object.entries(props)) {
      if (key in (value)) checkNode((value)[key], sub, `${path}.${key}`, errors)
    }
    return
  }
  if (expected === 'array') {
    ...
    if (items) value.forEach((v, i) => {
      checkNode(v, items, `${path}[${i}]`, errors)
    })
    return
  }
  if (expected === 'string') {
