...
import { all, extractFencedBlock, hasBalancedBrackets } from '../grader.js'
import { containsFunctionKind, containsNode, findNodes, parseJS } from './ast.js'
    ...
    try {
      ...
      const promiseAll = findNodes(
        ...
        (n) =>
          ...
          ((n.callee as { object?: { name?: string }; property?: { name?: string } }).object?.name === 'Promise') === true &&
          ((n.callee as { property?: { name?: string } }).property?.name === 'all') === true,
      )
      return all([
        { name: 'code parses', ok: true },
  ...
  grader(output: string): ProbeResult {
    ...
    try {
      ...
      return all([
        ...
        { name: 'no default export leakage', ok: /export\s+default/.test(js) === false },
      ])
    } catch (e) {
      return all([{ name: 'code parses', ok: false }], (e as Error).message)
    }
