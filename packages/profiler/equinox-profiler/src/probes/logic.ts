...
import { all, extractJSON, isArray, isInteger, isPlainObject, isString, parseJSONStrict } from '../grader.js'
...
const log01: Probe = {
  ...
  grader(output: string): ProbeResult {
    ...
    try {
      ...
      regions = isPlainObject(parsed) && isPlainObject(parsed['regions']) ? (parsed['regions'] as Record<string, unknown>) : {}
    } catch (e) {
...
const log02: Probe = {
  ...
  grader(output: string): ProbeResult {
    ...
    const checks = [
      ...
      { name: 'sum equals 17', ok: isArray(subset) && subset.reduce((s: number, v) => s + (isInteger(v) ? (v as number) : 0), 0) === 17 },
...
const log03: Probe = {
  ...
  grader(output: string): ProbeResult {
    ...
    const tasks = plan.map((p): { task: string; start: number } | null => {
      ...
      return { task: p['task'] as string, start: p['start'] as number }
    })
...
const log06: Probe = {
  ...
  grader(output: string): ProbeResult {
    ...
    for (const m of moves) {
      ...
      const [from, to] = [m[0] as string, m[1] as string]
      if (!(from in pegs) || !(to in pegs)) {
        legal = false
        break
      }
...
const log07: Probe = {
  ...
  grader(output: string): ProbeResult {
    ...
    const cells = path.map((p): [number, number] | null => (isArray(p) && isInteger(p[0]) && isInteger(p[1]) ? [p[0] as number, p[1] as number] : null))
    const walls = new Set(['0,1', '1,1'])
    const validCells = cells.every((c) => c !== null && c[0] >= 0 && c[0] < 3 && c[1] >= 0 && c[1] < 3 && !walls.has(`${c[0]},${c[1]}`))
    const starts = cells[0]?.[0] === 0 && cells[0]?.[1] === 0
    const ends = cells[cells.length - 1]?.[0] === 2 && cells[cells.length - 1]?.[1] === 2
    ...
    for (let i = 0; i < cells.length; i++) {
      ...
      if (i > 0) {
        ...
        const d = Math.abs(c[0] - prev[0]) + Math.abs(c[1] - prev[1])
        if (d !== 1) {
          stepsValid = false
          break
        }
...
const log08: Probe = {
  ...
  grader(output: string): ProbeResult {
    ...
    return all([
      ...
      { name: 'A + B = 9', ok: ints.length === 3 && ints[0]! + ints[1]! === 9 },
      { name: 'B + C = 14', ok: ints.length === 3 && ints[1]! + ints[2]! === 14 },
      { name: 'C > A', ok: ints.length === 3 && ints[2]! > ints[0]! },
...
const log09: Probe = {
  ...
  grader(output: string): ProbeResult {
    ...
    const items = slots.map((s): { talk: string; room: string; hour: number } | null => {
      ...
      return { talk: s['talk'] as string, room: s['room'] as string, hour: s['hour'] as number }
    })
    const valid = items.every((i) => i !== null && ['R1', 'R2'].includes(i!.room) && (i!.hour === 9 || i!.hour === 10))
    const keys = items.map((i) => `${i!.room}-${i!.hour}`)
    ...
    return all([
      ...
      { name: 'T1 and T2 at different hours', ok: t1 !== undefined && t2 !== undefined && t1!.hour !== t2!.hour },
