...
import SessionStore, { type Session, type SessionEvent } from '@solsticeai/equinox-session'
import InvariantRegistry, { InvariantError } from '@solsticeai/equinox-invariants'
import * as SandboxPolicyInvariant from '@solsticeai/equinox-sandbox-policy/invariant'
...
describe('sandbox-policy invariants', () => {
  ...
  it('rejects and attributes an unknown durable sandbox mode', async () => {
    ...
    expect(() => { ctx.emit('session/event', {} as Session, modeEvent('host-root')) })
      .toThrow(new InvariantError('@solsticeai/equinox-sandbox-policy', 'sandbox/mode carries unknown mode "host-root"'))
  })
  ...
  it('rejects an unknown mode already present on late registration', async () => {
    ...
    await expect(ctx.plugin(SandboxPolicyInvariant).then(() => undefined)).rejects.toMatchObject({
      code: 'INVARIANT',
      packageName: '@solsticeai/equinox-sandbox-policy',
    })
  })
})
