...
```yaml
- id: sandbox
  name: '@solsticeai/equinox-sandbox-windows-acl'
- id: sandbox-policy
...
- id: bash
  name: '@solsticeai/equinox-pwsh-sandbox'
```
...
- **Reads are unrestricted on Windows** — the ACL runner restricts writes only; the read boundary is documented in `@solsticeai/equinox-sandbox-windows-acl`.
