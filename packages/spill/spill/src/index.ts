 ...
 * `@solsticeai/equinox-spill-local` (host filesystem) is the first.
 *
 * The Service Definition is deliberately minimal: `saveText` and nothing else. It owns NO
 * retention policy (that is `@solsticeai/equinox-output-retention`), NO tool-result
 * replacement (that is `@solsticeai/equinox-spill-policy`), and NO retrieval or
 * search API. The backend supplies the locator and retrieval hint appropriate
 * for its storage substrate.
 *
 * @module @solsticeai/equinox-spill
 */
