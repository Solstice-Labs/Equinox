...
import { ToolCallId, ReasoningEffortId } from '@solsticeai/equinox-llm'
...
import { assembleContextFor, type Agent } from '@solsticeai/equinox-agent'
...
import type { SubagentStartRequest } from '@solsticeai/equinox-subagent'
...
describe('dsh-tool-subagent background mode', () => {
  ...
  it('fails loud when the tasks runtime is not loaded', async () => {
    ...
    expect(result.isError).toBe(true)
    expect(text(result)).toContain('background jobs unavailable: load @solsticeai/equinox-jobs')
  })
