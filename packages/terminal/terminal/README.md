...
Mount `@solsticeai/equinox-terminal` whenever a composition needs terminal sessions whose state survives across tool calls. The service alone does nothing useful: pair it with a backend such as `@solsticeai/equinox-terminal-bash` and a tool package such as `@solsticeai/equinox-tool-terminal`, and load all three in one composition.
...
Nothing directly. This package registers no prompt or tool; `@solsticeai/equinox-tool-terminal` owns visible schemas and result text.
...
No direct invalidation; `@solsticeai/equinox-tool-terminal` owns request-prefix changes.
