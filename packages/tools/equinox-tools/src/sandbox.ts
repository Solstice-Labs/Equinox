/** In-memory sandbox filesystem used by probe tool-flows and the ACI tools. */
...
export class SandboxFS {
  ...
  write(path: string, content: string): void {
    ...
    if (existing && existing.type === 'dir') throw new Error(`${name} is a directory`)
    dir.children!.set(name, { type: 'file', content })
  }
  ...
  private dir(parts: string[]): FsEntry {
    ...
    for (const part of parts) {
      ...
      if (!next || next.type !== 'dir') throw new Error(`no such directory: ${part}`)
      dir = next
    }
    return dir
  }
...
function projectEntries(entry: FsEntry): [string, FsEntry][] {
  return [...(entry.children ?? new Map<string, FsEntry>()).entries()]
}