...
import type { TypertContribution } from '@solsticeai/equinox-typert-registry/types'
import { EVENT_API, SERVICE_API, TYPE_API } from '@solsticeai/equinox-tool-cordis/src/api-catalog.ts'
...
describe('model-driven dsh-tools generation', () => {
  it('round-trips the complete service and event structure through the runtime registry', { timeout: 30_000 }, async () => {
    const workspace = new WorkspaceAnalyzer({
      root: workspaceRoot,
      faces: ['host'],
      packages: ['@solsticeai/equinox-tools'],
    }).analyze()
    ...
    const artifact = new FaceModelEmitter(host).emit('@solsticeai/equinox-tools')
    ...
    const record = ctx.typert.getPackage('@solsticeai/equinox-tools', 'host')
    ...
    await dispose()
    expect(ctx.typert.getPackage('@solsticeai/equinox-tools', 'host')).toBeUndefined()
  })
})
