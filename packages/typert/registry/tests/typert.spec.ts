...
} from '@solsticeai/equinox-typert-registry'
import type {
...
function toolsContribution(schema: z.ZodType = z.object({ name: z.string() })): TypertContribution {
  return {
    package: '@solsticeai/equinox-tools',
    face: 'host',
...
describe('TypertRegistry', () => {
  it('registers and queries generated schemas separately from package reflection', async () => {
    ...
    expect(typertKey('@solsticeai/equinox-tools', 'ToolInput')).toBe('@solsticeai/equinox-tools#ToolInput')
    expect(typertPackageKey('@solsticeai/equinox-tools', 'host')).toBe('@solsticeai/equinox-tools#host')
    expect(ctx.typert.get('@solsticeai/equinox-tools#ToolInput')).toMatchObject({
      package: '@solsticeai/equinox-tools',
      face: 'host',
      name: 'ToolInput',
    })
    expect(ctx.typert.get('@solsticeai/equinox-tools#ToolInput')?.schema).toBe(contribution.schemas[0]?.schema)
    expect(ctx.typert.getPackage('@solsticeai/equinox-tools', 'host')).toMatchObject({
      key: '@solsticeai/equinox-tools#host',
  ...
  it('withdraws schemas and package metadata through the exact contribution disposer', async () => {
    ...
    expect(ctx.typert.getPackage('@solsticeai/equinox-tools')).toBeDefined()

    await dispose()

    expect(ctx.typert.get('@solsticeai/equinox-tools#ToolInput')).toBeUndefined()
    expect(ctx.typert.getPackage('@solsticeai/equinox-tools')).toBeUndefined()
  ...
  it('rejects duplicate package faces and schema keys before committing', async () => {
    ...
    expect(ctx.typert.get('@solsticeai/equinox-tools#ToolInput')?.schema).toBe(original.schemas[0]?.schema)
  ...
  it('resolves required schemas and projects fresh JSON Schema documents', async () => {
    ...
    expect(ctx.typert.resolve('@solsticeai/equinox-tools#ToolInput').name).toBe('ToolInput')
    expect(() => ctx.typert.resolve('@solsticeai/equinox-tools#Missing')).toThrow('contributes no schema named "Missing"')
    ...
    const projected = ctx.typert.toJSONSchema('@solsticeai/equinox-tools#ToolInput')
    expect(projected).toMatchObject({ type: 'object', properties: { name: { type: 'string' } } })
    expect(ctx.typert.toJSONSchema('@solsticeai/equinox-tools#ToolInput')).not.toBe(projected)
  })
