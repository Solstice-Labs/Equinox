...
Python packages for driving Equinox as a subprocess. The client SDK communicates with the bundled runtime over newline-delimited JSON-RPC on stdio.

## Packages

| Directory | Dist / module | Role |
|---|---|---|
| [sdk](sdk/README.md) | `equinox-harness-sdk` / `equinox_harness` | High-level turns API and lower-level JSON-RPC client |
| [sdk-runtime](sdk-runtime/README.md) | `equinox-harness-runtime-bin` / `equinox_harness_runtime` | Bundled `dsh` CLI executable and native sidecars |
