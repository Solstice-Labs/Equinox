...
class HarnessConfig:
    """Configuration for launching the local Equinox SDK runtime."""
...
class HarnessClient:
    """Synchronous JSON-RPC client for the Equinox SDK runtime over stdio."""
    ...
    def close(self) -> None:
        ...
        if proc.poll() is None:
            try:
                ...
                proc.wait()
        self._proc = None
        self._fail_waiters(self._runtime_closed_error("Equinox runtime closed"))
            ...
            while True:
                ...
                if deadline is not None:
                    ...
                    if remaining <= 0:
                        ...
                        raise TimeoutError(
                            f"{method} timed out waiting for Equinox runtime{suffix}"
                        )
    ...
    def _write_message(self, message: JsonObject) -> None:
        ...
        if proc is None or proc.stdin is None:
            raise TransportClosedError("Equinox runtime is not running")
        try:
            ...
            raise self._runtime_closed_error("Failed to write to Equinox runtime") from exc
    ...
    def _reader_loop(self) -> None:
        ...
        try:
            ...
            self._fail_waiters(self._runtime_closed_error("Equinox runtime stdout closed"))
    ...
    def _default_launch_args(self, env: dict[str, str]) -> tuple[str, ...]:
        if self.config.dsh_bin is None:
            try:
                from equinox_harness_runtime import resolve_bundled_launch_args
            except ImportError as exc:
                raise FileNotFoundError(
                    "Unable to locate the bundled Equinox dsh runtime. "
                    "Install equinox-harness-runtime-bin."
                ) from exc
