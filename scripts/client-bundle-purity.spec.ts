...
const REQUESTING_PACKAGE = '@solsticeai/equinox-client-ui-conversation'
...
describe('client bundle build faces', () => {
  it('watches source in development and consumes emitted JavaScript in the Client build', () => {
    const bundle = clientBundle('@solsticeai/equinox-client-test', ['lib/types/index.js'])
...
describe('client bundle purity gate', () => {
  ...
  it('leaves default externals and non-scoped specifiers alone', () => {
    expect(resolveId('@solsticeai/equinox-client-store')).toBeNull()
    expect(resolveId('@solsticeai/equinox-client-ui-slots')).toBeNull()
    expect(resolveId('@solsticeai/equinox-client-ui-primitives')).toBeNull()
    expect(resolveId('react')).toBeNull()
    expect(resolveId('zod')).toBeNull()
  })

  it('rejects the retired web-react platform package', () => {
    expect(() => resolveId('@solsticeai/equinox-client-web-react')).toThrow(/purity/)
    expect(() => resolveId('@solsticeai/equinox-client-web-react/store')).toThrow(/purity/)
  })

  it('lets inline-safe libraries inline', () => {
    expect(resolveId('@solsticeai/equinox-session/surface')).toBeNull()
    expect(resolveId('@solsticeai/equinox-brand')).toBeNull()
    expect(resolveId('@solsticeai/equinox-deque')).toBeNull()
    expect(resolveId('@solsticeai/equinox-util-values')).toBeNull()
    expect(resolveId('@solsticeai/equinox-token-meter/client')).toBeNull()
    expect(() => resolveId('@solsticeai/equinox-token-meter')).toThrow(/purity/)
    expect(() => resolveId('@solsticeai/equinox-token-meter/client/internal')).toThrow(/purity/)
  })

  it('lets exact generated Remote contributions inline without admitting their package implementation', () => {
    expect(resolveId('@solsticeai/equinox-goal/remote')).toBeNull()
    expect(() => resolveId('@solsticeai/equinox-goal')).toThrow(/purity/)
    expect(() => resolveId('@solsticeai/equinox-goal/client')).toThrow(/purity/)
  ...
  it('throws on cross-plugin value imports — bare plugin names and /client subpaths alike', () => {
    expect(() => resolveId('@solsticeai/equinox-client-connection')).toThrow(/purity/)
    expect(() => resolveId('@solsticeai/equinox-client-ui-session')).toThrow(/purity/)
    expect(() => resolveId('@solsticeai/equinox-client-ui-layout/client')).toThrow(/purity/)
  })

  it('admits package-specific requests only for the declaring bundle', () => {
    const requesting = purityResolveId('@solsticeai/equinox-api-session-controller')
    expect(requesting('@solsticeai/equinox-api-gateway/client')).toBeNull()
    expect(() => resolveId('@solsticeai/equinox-api-gateway/client')).toThrow(/purity/)
  })
  ...
  it('externalizes the baseline independently of each package manifest', () => {
    ...
    const plain = clientConfigs('@solsticeai/equinox-client-connection')[0]?.deps as {
    ...
    expect(plain.neverBundle('@solsticeai/equinox-client-store')).toBe(true)
  })
})

describe('client bundle module requests', () => {
  it('requests what the declaration lists', () => {
    const requests = requestedExternals('@solsticeai/equinox-client-fixture', {
      external: ['react', 'react/jsx-runtime', '@solsticeai/equinox-client-ui-slots'],
    })

    expect([...requests].sort()).toEqual([
      '@solsticeai/equinox-client-ui-slots', 'react', 'react/jsx-runtime',
    ])
  })

  it('requests nothing when the declaration is absent', () => {
    expect(requestedExternals('@solsticeai/equinox-client-fixture', {}).size).toBe(0)
  })

  it('rejects a malformed declaration instead of reading past it', () => {
    expect(() => requestedExternals('@solsticeai/equinox-client-fixture', { external: 'react' }))
...
describe('client bundle debug artifacts', () => {
  ...
  it('maps first-party sources to their repository package paths', () => {
    const configs = clientConfigs('@solsticeai/equinox-client-ui-goal')
    ...
    const resolved = new URL(source, 'https://dsh.test/plugins/@solsticeai/equinox-client-ui-goal/client.js.map')
  ...
  it('maps dual-face host sources to the host package group', () => {
    const configs = clientConfigs('@solsticeai/equinox-host-directory-picker-native')
    const outputOptions = configs[0]?.outputOptions
    if (typeof outputOptions !== 'object' || outputOptions === null) throw new Error('client output options missing')
    const transform = outputOptions.sourcemapPathTransform
    if (transform === undefined) throw new Error('client sourcemap path transform missing')
  ...
  it('maps inlined workspace sources to packages and leaves dependencies outside it unchanged', () => {
    const configs = clientConfigs('@solsticeai/equinox-client-connection')
    const outputOptions = configs[0]?.outputOptions
    if (typeof outputOptions !== 'object' || outputOptions === null) throw new Error('client output options missing')
    const transform = outputOptions.sourcemapPathTransform
    if (transform === undefined) throw new Error('client sourcemap path transform missing')
    ...
    const resolved = new URL(workspaceSource, 'https://dsh.test/plugins/@solsticeai/equinox-client-connection/client.js.map')
