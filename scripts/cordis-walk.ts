 ...
 * BLOCK — a file may legally hold several `declare module '@solsticeai/cordis'` blocks
...
/** Every cordis module-merge body in `sf`: `declare module '@solsticeai/cordis'` (harness
...
function cordisModuleBodies(sf: ts.SourceFile): ts.ModuleBlock[] {
  ...
  for (const stmt of sf.statements) {
    ...
    if (stmt.name.text !== '@solsticeai/cordis' && stmt.name.text !== './context.ts') continue
...
/**
 * Every `key: Type` property a `declare module '@solsticeai/cordis'` Context merge
...
/**
 * Every event name a `declare module '@solsticeai/cordis'` Events merge declares in one
