...
/** Drop the `@solsticeai/equinox-` prefix so rows stay readable. */
function shortPackage(name: string): string {
  return name.replace('@solsticeai/equinox-', '')
}
...
export function renderClientCatalog(entries: readonly SlotEntry[]): string {
  const lines: string[] = [
    ...
    ' *',
    ' * @module @solsticeai/equinox-cordis-client-runner/client/slot-catalog',
    ' */',
    '',
    '/* jscpd:ignore-start */',
