...
const SESSION_PACKAGE = '@solsticeai/equinox-session'

/** The type-only module that plugin declaration merges augment. */
const SESSION_TYPES_MODULE = '@solsticeai/equinox-session/types'
 ...
 * top-level declaration (in `@solsticeai/equinox-session`) and any declaration
 * merge inside a `declare module '@solsticeai/equinox-session/types'` block. Both forms
...
export function renderKnownEventTypes(events: AnnotatedLogEventEntry[]): string {
  ...
  return [
    ...
    ' * @module @solsticeai/equinox-session/known-event-types',
    ' */',
    '',
    '/**',
