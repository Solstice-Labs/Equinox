...
describe('generated tsconfig package aliases', () => {
  it('maps each package to its own source directory', () => {
    ...
    const session = aliases.find(alias => alias.specifier === '@solsticeai/equinox-session')
    expect(session).toEqual({
      specifier: '@solsticeai/equinox-session',
    ...
    expect(aliases.some(alias => alias.specifier === '@solsticeai/equinox-typert-protocol')).toBe(false)
  })

  it('yields to a hand-written alias and closes without a trailing comma', () => {
    const aliases = [
      { specifier: '@solsticeai/equinox-a', source: './packages/g/a/src', hasInvariant: true },
      { specifier: '@solsticeai/equinox-b', source: './packages/g/b/src', hasInvariant: false },
    ]
    const body = renderAliases(aliases, new Set(['@solsticeai/equinox-a']))

    // The hand-written bare alias is skipped; its /invariant sibling is not.
    expect(body).toBe([
      '      "@solsticeai/equinox-a/invariant": ["./packages/g/a/src/invariant.ts"]',
      '      "@solsticeai/equinox-b": ["./packages/g/b/src"]',
    ].join(',\n'))
  ...
  it('names a package that no alias covers', () => {
    ...
    expect(uncoveredPackages(
      ['@solsticeai/equinox-a', '@solsticeai/equinox-b'],
      new Set(['@solsticeai/equinox-a', '@solsticeai/equinox-a/invariant']),
    )).toEqual(['@solsticeai/equinox-b'])

    expect(uncoveredPackages(['@solsticeai/equinox-a'], new Set(['@solsticeai/equinox-a']))).toEqual([])
  })
  ...
  it('covers every workspace package in the committed config', () => {
    ...
    expect(names).toContain('@solsticeai/equinox-typert-protocol')
  ...
  it('leaves no wildcard that probes every package group', () => {
    ...
    expect(config).not.toContain('"@solsticeai/equinox-*":')
    expect(config).not.toContain('"@solsticeai/equinox-*/invariant":')
  })
})
