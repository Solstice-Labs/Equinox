...
describe('collectPackageGraph', () => {
  ...
  it('rejects a missing in-repo peer', () => {
    ...
    expect(() => collectPackageGraph(root, ['client'], 'fixture'))
      .toThrow('fixture: @solsticeai/equinox-consumer references missing in-repo peer @solsticeai/equinox-missing')
  })
})
...
describe('renderModuleGraph', () => {
  it('renders the same peer edge in both generated languages', () => {
    const packages = [
      { short: 'provider', name: '@solsticeai/equinox-provider', group: 'core', rel: 'packages/core/provider', deps: [] },
      { short: 'consumer', name: '@solsticeai/equinox-consumer', group: 'core', rel: 'packages/core/consumer', deps: ['provider'] },
    ]
