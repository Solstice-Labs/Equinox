...
function checkManifest(
  ...
  if (owner.packageName === '@solsticeai/equinox-invariants') return
  ...
  const peerRange = manifest.peerDependencies?.['@solsticeai/equinox-invariants']
  if (developmentOnlyInvariant ? peerRange !== undefined : peerRange !== expectedRange) {
    addViolation(violations, owner.manifestPath, developmentOnlyInvariant
      ? '@solsticeai/equinox-invariants must not be a peerDependency under this package dependency policy'
      : '@solsticeai/equinox-invariants must be a workspace:^ peerDependency')
  }
  if (manifest.devDependencies?.['@solsticeai/equinox-invariants'] !== expectedRange) {
    addViolation(
      violations,
      owner.manifestPath,
      `@solsticeai/equinox-invariants must be a ${expectedRange} devDependency`,
    )
  }
}
...
function checkBuild(
  ...
  if (hasCompanion
    && owner.packageName !== '@solsticeai/equinox-invariants'
