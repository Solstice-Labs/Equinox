...
describe('release families', () => {
  ...
  it('accepts independent vendored versions and rejects an unpublishable one', () => {
    const vendor = releaseFamily('vendor')
    const members = [
      { ...member('vendor/cordis', '@solsticeai/cordis'), version: '4.0.1' },
      { ...member('vendor/cosmokit', '@solsticeai/cosmokit'), version: '1.8.2' },
    ]
  ...
  it('publishes a dependency before its consumer, and orders ties by name', () => {
    ...
    expect(dsh.publishOrder(members).order.map(entry => entry.name)).toEqual([
      '@solsticeai/equinox-library',
      '@solsticeai/equinox-consumer',
      '@solsticeai/equinox-zebra',
    ])
  })
  ...
  it('orders around a peer cycle rather than refusing to publish, and reports the edge it dropped', () => {
    ...
    expect(plan.order.map(entry => entry.name)).toEqual([
      '@solsticeai/equinox-right',
      '@solsticeai/equinox-left',
    ])
    // One of the two edges has to give, and which one it is belongs in the log.
    expect(plan.droppedPeerEdges).toEqual([
      { consumer: '@solsticeai/equinox-right', peer: '@solsticeai/equinox-left' },
    ])
  })
  ...
  it('honours an install edge even when a peer cycle surrounds it', () => {
    ...
    const members = [
      ...
      member('packages/a/consumer', '@solsticeai/equinox-consumer', {
        dependencies: { '@solsticeai/equinox-base': 'workspace:^' },
        peerDependencies: { '@solsticeai/equinox-base': 'workspace:^' },
      }),
    ]
    ...
    expect(plan.droppedPeerEdges).toEqual([
      { consumer: '@solsticeai/equinox-base', peer: '@solsticeai/equinox-consumer' },
    ])
  })
  ...
  it('refuses an order that would publish a consumer before a dependency it installs', () => {
    ...
    const members = [
      ...
      member('packages/a/charlie', '@solsticeai/equinox-charlie', { dependencies: { '@solsticeai/equinox-alpha': 'workspace:^' } }),
    ]
...
describe('payload change judgement', () => {
  const sourceShipping = member('vendor/cosmokit', '@solsticeai/cosmokit', {
    files: ['lib/index.js', 'lib/types/**/*.d.ts', 'src'],
  })
  const buildOutputOnly = member('vendor/cordis', '@solsticeai/cordis', {
  ...
  it('ignores paths no tarball carries', () => {
    ...
    expect(reachesPayload(member('packages/a/library', '@solsticeai/equinox-library', { files: ['lib/index.js'] }),
